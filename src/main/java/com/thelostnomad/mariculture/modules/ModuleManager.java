package com.thelostnomad.mariculture.modules;

import com.google.common.collect.Lists;
import com.thelostnomad.mariculture.Mariculture;
import com.thelostnomad.mariculture.api.MaricultureAPI;
import com.thelostnomad.mariculture.core.helpers.ConfigHelper;
import com.thelostnomad.mariculture.core.helpers.MCReflectionHelper;
import com.thelostnomad.mariculture.core.util.annotation.MCApiImpl;
import com.thelostnomad.mariculture.core.util.annotation.MCEvents;
...
public class ModuleManager {
    ...
    private static void initAPI(@Nonnull ASMDataTable table, Set<String> enabled) {
        Set<ASMDataTable.ASMData> datas = new HashSet<>(table.getAll(MCApiImpl.class.getCanonicalName()));
    ...
    private static void registerEventHandlers(@Nonnull ASMDataTable asmDataTable, boolean isClient) {
        ...
        Set<ASMDataTable.ASMData> asmDatas = new HashSet<>(asmDataTable.getAll(annotationClassName));
        for (ASMDataTable.ASMData asmData : asmDatas) {
            try {
                Map<String, Object> data = asmData.getAnnotationInfo();
    ...
    public static void loadConfigs() {
        for (Map.Entry<String, Class<?>> entry: enabled.entrySet()){
            try {
    ...
    private static boolean isModuleEnabled(String type, String name, boolean isEnabledByDefault) {
        ...
        try {
            ...
            Mariculture.logger.log(Level.ERROR, Mariculture.MODNAME + " failed to load it's " + type.toLowerCase() + " config");
            e.printStackTrace();
        } finally {
            config.save();
        }
